...
{
    /// <summary>
    /// Represents an iOS implementation of an <see cref="INativeFontFamily"/>.
    /// </summary>
    ...
    public class FontFamily : INativeFontFamily
    {
        ...
        /// </summary>
        public UIFontDescriptorSymbolicTraits Traits { get; }
        ...
        {
            Name = UIFont.FamilyNames.FirstOrDefault(n => n.Equals(familyName, StringComparison.InvariantCultureIgnoreCase)) ?? UIFont.PreferredBody.FamilyName;

            Traits = UIFont.PreferredBody.FontDescriptor.SymbolicTraits;
            if (traits != null)
            {
                ...
                for (int i = 0; i < traitArray.Length; i++)
                {
                    ...
                    if (Enum.TryParse<UIFontDescriptorSymbolicTraits>(traitArray[i], true, out trait))
                    {
                        Traits |= trait;
                    }
                }
